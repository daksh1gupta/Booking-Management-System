...
import { Mic, Send, X, Bot, MicOff } from "lucide-react";
...
const quickPrompts = ["Book a flight", "Show cheapest flights", "My bookings", "Flight status"];
...
const getResponse = (input: string): string => {
  ...
  return "I understand you're looking for help. Could you tell me more about what you need? I can help with booking flights, checking status, or finding deals!";
};
...
const AIChatAssistant = ({ isOpen, onToggle }: AIChatAssistantProps) => {
  const [messages, setMessages] = useState<Message[]>([
    { id: 0, text: "Hello 👋 I'm your Flight Booking Assistant. How can I help you today?", isBot: true },
  ]);
  ...
  const sendMessage = (text: string) => {
    if (!text.trim()) return;
    const userMsg: Message = { id: idRef.current++, text, isBot: false };
    setMessages((prev) => [...prev, userMsg]);
    setInput("");

    setTimeout(() => {
      const botMsg: Message = { id: idRef.current++, text: getResponse(text), isBot: true };
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 gradient-sky">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-primary-foreground">Flight Assistant 🤖</span>
              </div>
              <button onClick={onToggle} className="text-primary-foreground/70 hover:text-primary-foreground transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
